...
/*   Updated: 2025/03/25 18:12:23 by nhara            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	send_byte(pid_t pid, uint8_t byte)
		// 1バイト（8ビット）のデータを、ビット単位でサーバー（pid）に送信
{
	unsigned int bit_count = sizeof(byte) * 8;
		// bit_count は「あと何ビット送信するか」のカウンター。8を掛けてバイトからビットに変換

	while (bit_count--)
	{
		usleep(BIT_SEND_INTERVAL);      
			//シグナルが連続して送信されると受信側が処理できなくなるため、数マイクロ秒待機する
